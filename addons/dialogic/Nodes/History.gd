...
func handle_scrollbar_changed():
	#It's firing every frame, we only want to check it once checked opening 
	if(scrollToggle):
		scrollToggle = false
		if (scrollToBottom):
...
# Add history based checked the passed event, using some logic to get it right
...
# Used to manually toggle the history visibility checked or unchecked
