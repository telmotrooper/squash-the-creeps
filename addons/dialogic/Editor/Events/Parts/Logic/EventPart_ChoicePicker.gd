...
func _ready():
	# e.g. 
	input_field.connect("text_changed",Callable(self,"_on_ChoiceText_text_changed"))
	condition_picker.connect("data_changed",Callable(self,"_on_ConditionPicker_data_changed"))
	condition_picker.connect("remove_warning",Callable(self,"emit_signal").bind("remove_warning"))
	condition_picker.connect("set_warning",Callable(self,"set_warning"))
