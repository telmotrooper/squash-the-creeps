...
func _ready():
	custom_slot.connect("text_changed",Callable(self,'_on_CustomSlot_text_changed'))
	slot_picker.get_popup().connect("index_pressed",Callable(self,"on_SlotPicker_index_pressed"))
