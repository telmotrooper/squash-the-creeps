...
func _ready():
	seconds_selector.connect("value_changed",Callable(self,"_on_SecondsSelector_value_changed"))
	skippable_selector.connect("toggled",Callable(self,"_on_SkippableSelector_toggled"))
	hideBox_selector.connect("toggled",Callable(self,"_on_HideDialogBox_toggled"))
...
func load_data(data:Dictionary):
	...
	skippable_selector.button_pressed = event_data.get('waiting_skippable', false)
	hideBox_selector.button_pressed = event_data.get('hide_dialogbox', true)
