...
func _ready():
	...
	fade_duration.connect('value_changed',Callable(self,'_on_fade_duration_changed'))
...
func _on_FilePicker_data_changed(data):
	...
	fade_duration.visible = !data['background'].is_empty()
	fade_duration_label.visible = !data['background'].is_empty()

	if !data['background'].is_empty():
