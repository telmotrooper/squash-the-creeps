...
func _on_item_selected():
	...
	match metadata['editor']:
		'Timeline':
			# Remember to also update this checked the `inspector_timeline_picker.gd`
...
## *****************************************************************************
##					 CONTEXT POPUPS checked RMB SELECT
## *****************************************************************************
...
func _on_TimelineRootPopupMenu_id_pressed(id):
	...
	if id == 1: # add subfolder
		...
		build_timelines()
	if id == 2: # remove_at folder and substuff
...
func _on_DefinitionRootPopupMenu_id_pressed(id):
	...
	if id == 2: # add subfolder
		DialogicUtil.add_folder(get_item_path(get_selected()), "New Folder "+str(Time.get_unix_time_from_system()))
		build_definitions()
	if id == 3: # remove_at folder and substuff
		if get_selected().get_parent() == get_root():
			return
		editor_reference.get_node('RemoveFolderConfirmation').popup_centered()
...
func save_current_resource():
	if editor_reference and editor_reference.visible: #Only save if the editor is open
		...
		if item != null:
			...
			# Note: Theme files auto saves checked change
