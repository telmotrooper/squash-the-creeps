...
func _process_transforms(transforms, global_seed) -> void:
	...
	for i in transforms.list.size():
		...
		if local_space:
			t = t.rotated(t.basis.x.normalized(), deg_to_rad(_random_float() * rotation.x))
			t = t.rotated(t.basis.y.normalized(), deg_to_rad(_random_float() * rotation.y))
			t = t.rotated(t.basis.z.normalized(), deg_to_rad(_random_float() * rotation.z))
			t.origin = origin + t * _random_vec3() * position

		else:
			t = t.rotated(global_x, deg_to_rad(_random_float() * rotation.x))
			t = t.rotated(global_y, deg_to_rad(_random_float() * rotation.y))
			t = t.rotated(global_z, deg_to_rad(_random_float() * rotation.z))
