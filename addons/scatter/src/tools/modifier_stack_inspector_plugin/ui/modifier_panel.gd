...
func _ready() -> void:
	...
	_margin_container.connect("resized",Callable(self,"_on_child_resized"))
...
func create_ui_for(modifier) -> void:
	_modifier = modifier
	_modifier.connect("warning_changed",Callable(self,"_on_warning_changed"))
	_on_warning_changed()
	...
	for property in modifier.get_property_list():
		...
		match property.type:
			...
			TYPE_STRING:
				...
				elif property.hint_string == "File" or property.hint_string == "Texture2D":
		...
		if parameter_ui:
			...
			parameter_ui.connect("value_changed",Callable(self,"_on_parameter_value_changed").bind(property.name, parameter_ui))
